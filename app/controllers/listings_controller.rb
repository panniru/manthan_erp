    ...
    respond_to do |format|
      format.json  do
        ...
        @list = Listing.where(:grade_master_id => a, :subject_master_id => b).map do |x|
          {listing_id: x.id,grade_name: x.grade_master.grade_name, subject_name: x.subject_master.subject_name, assessment_date: x.assessment_date, assessment_description: x.assessment_description, assessment_name: x.assessment_name, grade_master_id: x.grade_master_id}
        end
        render :json => @list
      end
    end
